...
15. `cache.ttl` 必须大于 0
16. App 至少启用一种投递方式：SSE 或 HTTP Callback
17. 启用 SSE 的 App 必须配置 `token`
18. 启用 HTTP Callback 的 App 必须配置 `delivery.callback.url`
19. Callback `timeout`、`max_attempts`、`initial_backoff`、`max_backoff` 必须大于 0
